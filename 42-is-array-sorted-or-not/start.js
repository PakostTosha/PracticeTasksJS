...
const isSortedUp = (arr) => arr.every((v, i, a) => !i || a[i - 1] <= v);
const isSortedDown = (arr) => arr.every((v, i, a) => !i || a[i - 1] >= v);

const arraySortInfo = (inputArray) => {
  let isSortedUpFlag;
  let isSortedDownFlag;
  if (inputArray.every((elem) => typeof elem == "number")) {
    isSortedDownFlag = isSortedDown(inputArray);
    isSortedUpFlag = isSortedUp(inputArray);
    return (
      (isSortedUpFlag && "Массив отсортирован по возрастанию") ||
      (isSortedDownFlag && "Массив отсортирован по убыванию") ||
